 ...
 * make an interruptible version that pickles state uppon SIGHUP, SIGTERM, etc
'''
...
def trycept(f, *rgs, default = None, raize = False, **nargs):
    try: f(*args, **nargs)
...
def gen_edges(L:int) -> Dict[bytes,bytes]:
    with mp.Pool(mp.cpu_count() * 3 // 4) as p:
        ...
        pickle.dump(edges, f'hendo/{L}_full.pkl')
        return edges
